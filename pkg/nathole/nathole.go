...
// PreCheck 用于检查代理是否已准备好进行渗透。
// 在调用Prepare之前调用此函数，以避免不必要的准备工作。
func PreCheck(
...
// ExchangeInfo 用于客户端和访问者之间交换信息。
// 1. 通过 msgTransporter 向服务器发送输入消息。
// 2. 服务器将收集来自客户端和访问者的信息并进行分析。然后向他们发回 NatHoleResp 消息，告诉他们下一步该怎么做。
