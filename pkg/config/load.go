...
import (
	...
	"github.com/xx/xxx/pkg/config/legacy"
...
// LoadFileContentStringWithTemplate
func LoadFileContentStringWithTemplate(configStr string, values *Values) ([]byte, error) {
	b := []byte(configStr)
	return RenderWithTemplate(b, values)
}

func LoadConfigureFromFile(path string, c any, strict bool, flag bool) error {
	var content []byte
	var err error
	if flag {
		content, err = LoadFileContentStringWithTemplate(path, GetValues())
	} else {
...
func LoadServerConfig(path string, strict bool) (*v1.ServerConfig, bool, error) {
	...
	if DetectLegacyINIFormatFromFile(path) {
		...
		svrCfg = &v1.ServerConfig{}
		if err := LoadConfigureFromFile(path, svrCfg, strict, false); err != nil {
...
func LoadClientConfig(path string, strict, flag bool) (
	*v1.ClientCommonConfig,
	...
	//if DetectLegacyINIFormatFromFile(path) { // 从文件中检测遗留ICI格式
	...
	//	if err := LoadConfigureFromFile(path, &allCfg, strict, flag); err != nil {
	...
	// 从包含内容中加载附加配置 旧版 ini 格式已在 ParseClientConfig 中处理此问题.
	...
	for _, path := range paths {
		...
		for _, fi := range files {
			...
			if matched, _ := filepath.Match(filepath.Join(absDir, filepath.Base(path)), absFile); matched {
				...
				cfg := v1.ClientConfig{}
				if err := LoadConfigureFromFile(absFile, &cfg, strict, false); err != nil {
