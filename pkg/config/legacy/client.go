...
import (
	...
	legacyauth "github.com/xx/xxx/pkg/auth/legacy"
	"github.com/xx/xxx/pkg/util/util"
)

// ClientCommonConf 是从 ini 解析的配置。
// 它包含客户端服务的信息。建议使用 GetDefaultClientConf 而不是直接创建此对象，以便所有未指定的字段都有合理的默认值。
type ClientCommonConf struct {
	legacyauth.ClientConfig `ini:",extends"`

	// ServerAddr 指定要连接的服务器的地址。默认情况下，此值为“0.0.0.0”
	ServerAddr string `ini:"server_addr" json:"server_addr"`
	// ServerPort 指定连接服务器的端口。默认情况下此值为 7000
	...
	// 默认情况下，此值为空。
	// 此值仅在 TCP/Websocket 协议中使用。KCP 协议不支持。
	ConnectServerLocalIP string `ini:"connect_server_local_ip" json:"connect_server_local_ip"`
	// HTTPProxy 指定要通过其连接到服务器的代理地址。如果
	// 此值为“”，则将直接连接到服务器。默认情况下，
	// 此值从“http_proxy”环境变量中读取。
	HTTPProxy string `ini:"http_proxy" json:"http_proxy"`
	// LogFile 指定将写入日志的文件。此值将
	// 仅在 LogWay 设置正确时使用。默认情况下，此值为 “console”。
	LogFile string `ini:"log_file" json:"log_file"`
	// LogWay 指定日志管理方式。有效值为“console”
	// 或“file”。如果使用“console”，日志将打印到 stdout。如果
	// 使用“file”，日志将打印到 LogFile。默认情况下，此值 为“console”。
